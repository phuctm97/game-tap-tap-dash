...
#include "PlayScene/Maps/PlayScene_FlyGameMapNode_2.h"
...
namespace TestScene
{
	...
	bool TestMapNodeScene::onTouchBegan(cocos2d::Touch* touch, cocos2d::Event* e)
	{
		...
		case 0: {
			...
			if (node->checkPositionInside(touch->getLocation()))
				CCLOG("True");
		...
		case 4: {
			...
			auto node = PlayScene::FlyGameMapNode2::create();
			node->setPosition(Director::getInstance()->getVisibleSize().width * 0.5f,
				Director::getInstance()->getVisibleSize().height * 0.5f);
			node->runAction(RepeatForever::create(RotateBy::create(1.0f, 360)));
		...
		case 5: {
			...
			auto node = PlayScene::ForwardGameMapNodeWithRest::create(15, 100);
			node->setPosition(Director::getInstance()->getVisibleSize().width * 0.5f,
				Director::getInstance()->getVisibleSize().height * 0.5f);
			node->runAction(RepeatForever::create(RotateBy::create(1.0f, 360)));
